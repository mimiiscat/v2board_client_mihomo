...
if (typeof process !== 'undefined' && process.versions?.electron) {
  window.__ELECTRON__ = {
    ...
    checkCoupon: (code, planId) => window.require('electron').ipcRenderer.invoke('check-coupon', code, planId),
    fetchPaymentMethods: () => window.require('electron').ipcRenderer.invoke('fetch-payment-methods'),
    createOrder: (payload) => window.require('electron').ipcRenderer.invoke('create-order', payload),
    checkoutOrder: (payload) => window.require('electron').ipcRenderer.invoke('checkout-order', payload),
    openExternal: (url) => window.require('electron').ipcRenderer.invoke('open-external', url),
    copyText: (text) => window.require('electron').ipcRenderer.invoke('copy-text', text),
